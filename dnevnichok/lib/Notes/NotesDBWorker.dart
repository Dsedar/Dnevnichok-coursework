...
class NotesDBWorker {
  ...
  static final NotesDBWorker db = NotesDBWorker._internal();
  ...
  Note noteFromMap(Map inMap) {
    ...
    note.mood = inMap["mood"];
    note.noteDate = inMap["noteDate"];
    note.noteTime = inMap["noteTime"];
  ...
  Map<String, dynamic> noteToMap(Note inNote) {
    ...
    map["mood"] = inNote.mood;
    map["noteDate"] = inNote.noteDate;
    map["noteTime"] = inNote.noteTime;
  ...
  Future create(Note inNote) async {
    ...
    return await db.rawInsert(
        "INSERT INTO notes (id, title, content, mood, noteDate, noteTime) VALUES (?, ?, ?, ?, ?, ?)",
        [
          id,
          inNote.title,
          inNote.content,
